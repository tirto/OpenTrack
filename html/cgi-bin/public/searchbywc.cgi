...
# User already entered names for searching. Now search.
	...
	if ($err == 0) {
		...
		my $entries = ListHeaderTitle('searchbywc.cgi', $result, $idx, $code);
		print '</form>';
...
use integer;
		...
		if ($pages > 0) {
			...
			print "Go to page: \n";
			foreach $i (0 .. $pages) {
				print "<a href=/cgi-bin/public/searchbywc.cgi?search=Search&opts=$code&idx=$i>$i";
				print "<\/a>\n";
				print "&nbsp&nbsp\n";
			}
			
			if ($remain > 1) {
				print "<a href=/cgi-bin/public/searchbywc.cgi?search=Search&idx=$i&opts=$code>$i";
				print "<\/a>\n";
			}
		}
...
use integer;
	...
	if ($err == 0) {
		...
		if (@$result > 1) {
			print '<form method=post enctype=application/x-www-form-urlencoded>';
			$entries = ListHeaderTitle('searchbywc.cgi', $result, $idx, $code);
			print '</form>';
...
# Output: None.
	...
	print "<Tr>\n";
	print "<Td><input type=checkbox name=wname value=last checked> Last name<\/td>\n";
	print "<Td><input type=checkbox name=wname value=first> First name<\/td>\n";
	print "<Td><input type=checkbox name=wname value=either> Either name<\/td>\n";
	print "<\/Tr>\n";
