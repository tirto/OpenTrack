...
# User already entered names for searching. Now search.
	...
	if ($err == 0) {
		...
		my $entries = ListHeaderTitle('searchbyname.cgi', $result, 0, $code, $page_size);
		print '</form>';
...
use integer;
		...
		if ($pages > 0) {
			...
			print "Go to page: \n";
			foreach $i (0 .. $pages) {
				print "<a href=/cgi-bin/Info/searchbyname.cgi?search=Search&opts=$code&idx=$i>$i";
				print "<\/a>\n";
				print "&nbsp&nbsp\n";
			}
...
use integer;
	...
	if ($err == 0) {
		...
		if (@$result > 1) {
			print '<form method=post enctype=application/x-www-form-urlencoded>';
			$entries = ListHeaderTitle('searchbyname.cgi', $result, $idx, $code, $page_size);
			print '</form>';
...
use integer;
			...
			if ($pages > 0) {
				...
				if ($remain > 1) {
					print "<a href=/cgi-bin/Info/searchbyname.cgi?search=Search&idx=$i&opts=$code>$i";
					print "<\/a>\n";
				}
			}
