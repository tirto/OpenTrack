...
# User already entered names for searching. Now search.
	...
	# Searching.
	($err, $result) = SearchForNA($code); 

	if ($err == 0) {
		print '<form method=post enctype=application/x-www-form-urlencoded>';
		$entries = ListHeaderTitle('searchforna.cgi', $result, $idx, $code, $page_size);
		print '</form>';
...
use integer;
		...
		if ($pages > 0) {
			...
			print "Go to page: \n";
			foreach $i (0 .. $pages) {
				print "<a href=/cgi-bin/Info/searchforna.cgi?search=Search&opts=$code&idx=$i>$i";
				print "<\/a>\n";
				print "&nbsp&nbsp\n";
			}
			
			if ($remain > 1) {
				print "<a href=/cgi-bin/Info/searchforna.cgi?search=Search&idx=$i&opts=$code>$i";
				print "<\/a>\n";
			}
		}
...
# A user wants to see details of the next record in the search result list.
	...
	# Searching.
	my ($err, $result) = SearchForNA(param('opts'));
	...
	if ($is_success == 0) {
		...
		print "<h2>The Name <a href=searchforna?search=Search&sfirstname=$f&slastname=$l&sbool=and>$l, $f<\/a> Already Exists<\/h2>\n";
		DetailForm('set', param('sid'), param('opts'));
	}
}
...
# Output: None.
	...
	print "<br><br><br>\n";
	print "<center>\n";
	print "<h2>Search For N/A Fields<\/h2>\n";
	print "<br><br>\n";
	print "<form method=post enctype=application/x-www-form-urlencoded>\n";
