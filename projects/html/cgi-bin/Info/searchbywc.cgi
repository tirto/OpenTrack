...
# User already entered names for searching. Now search.
	...
	if (param('opts')) {
		...
		$code = param('wcard') . "_" . param('wname');
	}
	...
	# Searching.
	($err, $result) = SearchByWildCard($code); 

	if ($err == 0) {
		print '<form method=post enctype=application/x-www-form-urlencoded>';
		my $entries = ListHeaderTitle('searchbywc.cgi', $result, $idx, $code, $page_size);
		print '</form>';
...
use integer;
		...
		if ($pages > 0) {
			...
			print "Go to page: \n";
			foreach $i (0 .. $pages) {
				print "<a href=/cgi-bin/Info/searchbywc.cgi?search=Search&opts=$code&idx=$i>$i";
				print "<\/a>\n";
				print "&nbsp&nbsp\n";
			}
			
			if ($remain > 1) {
				print "<a href=/cgi-bin/Info/searchbywc.cgi?search=Search&idx=$i&opts=$code>$i";
				print "<\/a>\n";
			}
		}
...
use integer;
	...
	if ($err == 0) {
		...
		if (@$result > 1) {
			print '<form method=post enctype=application/x-www-form-urlencoded>';
			$entries = ListHeaderTitle('searchbywc.cgi', $result, $idx, $code, $page_size);
			print '</form>';
...
# A user wants to see details of the next record in the search result list.
	...
	# Searching.
	my ($err, $result) = SearchByWildCard(param('opts'));
	...
	if ($is_success == 0) {
		...
		print "<h2>The Name <a href=searchbywc.cgi?search=Search&sfirstname=$f&slastname=$l&sbool=and>$l, $f<\/a> Already Exists<\/h2>\n";
		DetailForm('set', param('sid'), param('opts'));
	}
}
...
# Output: None.
	...
	print "<br><br><br>\n";
	print "<center>\n";
	print "<h2>Don't remember the name<\/h2>\n";
	print "<h3>type in one or first few characters<\/h3>\n";
	print "<br>\n";
	print "<form method=post enctype=application/x-www-form-urlencoded>\n";
	print "<input type=text name=wcard size=16 maxlength=16>\n";
	print "<br>\n";
	print "<table border=0 cellspace=10>\n";
	print "<Tr>\n";
	print "<Td><input type=radio name=wname value=last checked> Last name<\/td>\n";
	print "<Td><input type=radio name=wname value=first> First name<\/td>\n";
	print "<Td><input type=radio name=wname value=either> Either name<\/td>\n";
	print "<\/Tr>\n";
