...
if (param('delete')) {
  ...
  if (param('change_sta') eq 'true' && param('status') eq 'Active') {
    ...
    UpdateRequest($dbh, param('rid'), param('responder'), param('status'), param('priority'), param('title'), param('comments'), param('reassigned'), param('resol'),  param('sendnoteto'));
  }
...
# Output: None.
  ...
  if ($personassigned ne 'Nobody' && $personassigned ne $manageInfo[1]) {
    ...
    $sth = $dbh->prepare(qq{SELECT email, fullname FROM assignList
    ...
    print MAIL "You have new request. Request ID is $rid.\n";
  ...
  if ($sendnote) {
    ...
    print MAIL "Your request, ID number $rid, has been forwarded to $row[1].\n";
    print MAIL "You may email $row[1] at $row[0].\n";
    print MAIL "Thank you.\n";
    close (MAIL);
  } # End if ($sendnote)
  ...
  if ($status eq 'Finished' && $status ne $manageInfo[2]) {
    ...
    if (!$resolution) {
      print '<CENTER>';
      print h1("Please fill out the resolution field");
      print '</CENTER>';
      return;
    }
  ...
  print '<hr width=20%>';
  print '<center>';
  print h1("This request has been updated");
  print '</center>';
  print '<hr width=20%>', <br>;
} # End UpdateRequest
 ...
 if ($row[12] eq 'Finished') {
    ...
    print table({-align=>'center'}),
              ...
              Tr(td({-align=>'right'}, "Resolution : "),
                 td({-align=>'left', -colspan=>3}, textarea(-name=>"resol",
