...
# Start with an empty classmeetingproj table which will later
# be filled with classes and the room they are scheduled to.
$sth = $dbh->prepare(qq{DELETE FROM classmeetingproj});
$sth->execute or die "executing: $sth->errstr";
$sth->finish;

# Fill in the classmeetingproj table with data from classmeeting
# table. classmeeting is the orginal table and classmeetingproj
# table is the working table.
$sth1 = $dbh->prepare(qq{SELECT term, department, codenumber, section,
			 counter, count_total, days, starttime, stoptime
			     FROM classmeeting WHERE term = '$term' });
$sth1->execute or die "Executing: $sth1->errstr";

$sth2 = $dbh->prepare(qq{INSERT INTO classmeetingproj
			     values(?,?,?,?,?,?,?,?,?,?,?)});
...
# Select all classrooms(not labs) from room table
$sth = $dbh->prepare(qq{
    SELECT building, roomNumber, capacity
	FROM room WHERE category = 'classroom'
	    ORDER BY capacity DESC});
$sth->execute or die "executing: $sth->errstr";

# Create a bit array for each classroom with slots in
# 5 minutes increments stating whether it is free or not.
$roomIndex = 0;
while (@row = $sth->fetchrow_array) {
    $room[$roomIndex] = $row[0];			# Building
    $room[$roomIndex+1] = $row[1];			# Room Number
    $room[$roomIndex+2] = $row[2];			# Capacity
    $room[$roomIndex+3] = int($row[2] * 1.05);		# Overloaded capacity

    # Monday
    $room[$roomIndex+4] = '';				# string to store array of bits.                                                    
    vec($room[$roomIndex+4], 0, 32) = 0;		# 5h20 - 24h00 = 224 five-minute slots
    ...
    # Tuesday
    $room[$roomIndex+5] = '';
    vec($room[$roomIndex+5], 0, 32) = 0;
    vec($room[$roomIndex+5], 1, 32) = 0;
    vec($room[$roomIndex+5], 2, 32) = 0;
    vec($room[$roomIndex+5], 3, 32) = 0;
    vec($room[$roomIndex+5], 4, 32) = 0;
    vec($room[$roomIndex+5], 5, 32) = 0;
...
# Start the main processing look to update classmeetingproj
# Seems blank space padding is a problem below so I use 'rtrim' to ignore this
$sth3 = $dbh->prepare(qq{
    UPDATE classmeetingproj
	SET building = ?, roomNumber = ?
	    WHERE rtrim(term) = rtrim(?) AND rtrim(department) = rtrim(?)
		AND rtrim(codenumber) = rtrim(?) AND rtrim(section) = rtrim(?)
		AND rtrim(counter) = rtrim(?)});

$nbRooms = $roomIndex;

# Each day is considered seperately        
# Give priority to courses with highest enrollment,
# earliest start time and number of days of classes first
# since they are the hardest to satisfy.
for ($weekIndex = 0; $weekIndex < 7; $weekIndex++) {
    $sth = $dbh->prepare(qq{
	SELECT co.department, co.codeNumber, co.section, cl.counter,
	co.maxEnrollment, cl.startTime, cl.stopTime, cl.days,
	LENGTH(rtrim(cl.days)), cl.building, cl.roomNumber
		...
		WHERE co.term = '$term'
		    ...
		    AND co.activity <> 'LAB'
			ORDER BY co.maxEnrollment DESC,
			cl.startTime, LENGTH(cl.days) DESC});

    $sth->execute or die "executing: $sth->errstr";

    # Stored each course meeting for that day into meeting array
    $meetingIndex = 0;
    while (@row = $sth->fetchrow_array) {
	if ($row[7] !~ /TBA/ and $row[5] !~ /TBA/ and !defined $row[9]) {
	...
	# Unpack meeting start time into a slot index
	...
	# Unpack meeting stop time into a slot index
	...
	for ($roomIndex = 0; $roomIndex < $nbRooms; $roomIndex+=11) {
	    ...
	    if ($room[$roomIndex+2] < 0.90 * $meeting[$meetingIndex+4]) {
		$isGoodRoom = 0;
		# $roomIndex = $nbRooms; # We jump out of the $roomIndex loop (THIS MAKES NO SENSE!!!!!)
	    } else {
	...
	if ($conflict == 0) {
		...
		$dayIndicesCounter = 1;
		for ($dayIndex = $weekIndex+1; $dayIndex < 7; $dayIndex++) {
		    #We skip the first day, already calculated
    ...
    print Tr({-bgcolor=>"#11EE88"},td("Department"),td("Codenumber"),td("Section"),
