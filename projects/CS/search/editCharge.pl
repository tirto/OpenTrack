...
sub get_charge_entry($$$) {
    ...
    my ($dbh, $request_id, $charge_id) = @_;
    my ($sth, $row_ref, @row, $charge_ref);
	...
	# an existing charge
	$charge_ref->{'request_id'} = $row[0];
	$charge_ref->{'charge_id'} = $row[1];
	$charge_ref->{'order_date'} = $row[2];
	$charge_ref->{'invoice'} = $row[3];
	$charge_ref->{'description'} = $row[4];
	$charge_ref->{'vendor'} = $row[5];
	$charge_ref->{'amount'} = $row[6];
	$charge_ref->{'dept'} = $row[7];
	$charge_ref->{'payer'} = $row[8];
	$charge_ref->{'paytype'} = $row[9];
	$charge_ref->{'received'} = $row[10] || ' ';
	$charge_ref->{'accounted'} = $row[11] || ' ';	 
    } else {
	# a new charge so assign default values
	my $order_date = date_string(this_month(), this_day(), this_year());
	
	$charge_ref->{'request_id'} = $row_ref->[0][0];
	$charge_ref->{'charge_id'} = undef;
	$charge_ref->{'order_date'} = $order_date;
	$charge_ref->{'invoice'} = "";
	$charge_ref->{'description'} = "(none)";
	$charge_ref->{'vendor'} = "(none)";
	$charge_ref->{'amount'} = 0.00;
	$charge_ref->{'dept'} = 5;
	$charge_ref->{'payer'} = 'Ben';
	$charge_ref->{'paytype'} = 1;
	$charge_ref->{'received'} = ' ';
	$charge_ref->{'accounted'} = ' ';	
    }
