...
/*
 * search_name.php
 * for searching student course info database by name
 *
 * @version 1.0 
 * @date 01-16-00
 * @author Tirto Adji
...
if ($search) {
	...
	if ($fname!='' && $lname=='') {
	    ...
	    $clause = "cs_fname like '_$fname%'";
	}
	elseif ($lname!='' && $fname == '') {
            $lname = strtoupper($lname);
	    $clause = "cs_lname like '$lname%'";
	}
	elseif ($lname!='' && $fname != '') {
            $lname = strtoupper($lname);
	    $fname = strtoupper($fname); 
	    $clause = "cs_fname like '_$fname%' or cs_lname like '$lname%'";
	}
	else {
	    print "<br><br><br><br>Search failed! Must enter first or last name"; 
	    print "<br><br><a href=search_name.php>Search by Name Again</a>";
	    return;
	}    
    }
    elseif ($sbool=='and') {
	if ($fname=='' || $lname=='') {
	    print "<br><br><br><br>Search failed! Must enter first and last name"; 
	    print "<br><br><a href=search_name.php>Search by Name Again</a>";
	    return;
	}
	else {
            $lname = strtoupper($lname);
	    $fname = strtoupper($fname);
	    $clause = "cs_lname like '$lname%' and cs_fname like '%$fname%'";
	}    
    }	
     $tablename = "cs_students";
     $stmt = OCIParse($conn,"select cs_sid, cs_lname, cs_fname, cs_class from $tablename where $clause");
     OCIExecute($stmt);
...
?>
	...
	<TD><INPUT TYPE="text" NAME="fname"  VALUE='' SIZE=16 MAXLENGTH=16></TD>
	<TD>&nbsp</TD>
	<TD><INPUT TYPE="radio" NAME="sbool" VALUE="or" CHECKED> or <INPUT TYPE="radio" NAME="sbool" VALUE="and">and</TD>
	<TD>&nbsp</TD>
	<TD ALIGN="right">Last name:</TD>
	<TD><INPUT TYPE="text" NAME="lname"  VALUE='' SIZE=16 MAXLENGTH=16></TD>
    </TR>
    </table>
